...
class GPXExtractor {
  static async parseGPX(gpxString) {
  ...
  static extractCoordinates(gpxObj) {
    const coordinates = [];
  ...
  static async processGPXFile(filePath) {
    try {
      const gpxString = fs.readFileSync(filePath, "utf8");
      const gpxObj = await GPXExtractor.parseGPX(gpxString);
      const coordinates = GPXExtractor.extractCoordinates(gpxObj);
      return coordinates;
    } catch (error) {
